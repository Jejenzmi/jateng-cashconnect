...
export default function PatientAppointments() {
  ...
  const fetchData = async () => {
    try {
      // Get patient ID
      const patient = await getApi("/generic-api");

      if (patient.length > 0) {
        setPatientId(patient[0].id);

        // Fetch appointments
        const appointmentsData = await getApi("/generic-api");
        setAppointments(appointmentsData || []);
      }

      // Fetch doctors
      const doctorsData = await getApi("/generic-api");
