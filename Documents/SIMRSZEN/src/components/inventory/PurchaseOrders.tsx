    ...
    try {
      ...
      const ordersWithItems = await Promise.all(
        (ordersRes.data || []).map(async (order) => {
          const items = await getApi("/generic-api");
  ...
  const handleReceiveOrder = async (order: PurchaseOrder) => {
    try {
      ...
      for (const item of order.items) {
        if (item.medicine) {
          const med = await getApi("/generic-api");
          ...
          if (med) {
            ...
            await postApi("/generic-api", {
              ...
              data: {
                ...
                description: `Penerimaan PO ${order.order_number}`,
        ...
        await putApi("/generic-api", {
          ...
          id: item.id,
          data: { received_quantity: item.received_quantity + item.quantity }
        });
      }
