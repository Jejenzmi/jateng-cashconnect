    ...
    try {
      const result = await getApi<{
        ...
        SELECT 
          ...
          pat.medical_record_number AS patient_medical_record_number,
          d.full_name AS doctor_full_name
        FROM prescriptions p
        LEFT JOIN patients pat ON p.patient_id = pat.id
        LEFT JOIN doctors d ON p.doctor_id = d.id
        WHERE p.pickup_code = ${searchCode.toUpperCase()} OR p.qr_token = ${searchCode}
        LIMIT 1
      `;
      ...
      const items = await getApi<{
        ...
        WHERE pi.prescription_id = ${prescriptionData.id}
      `;
      ...
      setPrescription({
        ...
        pickup_code: prescriptionData.pickup_code,
        patient: prescriptionData.patient_full_name
          ? {
              full_name: prescriptionData.patient_full_name,
              medical_record_number: prescriptionData.patient_medical_record_number,
            }
          : null,
        doctor: prescriptionData.doctor_full_name
          ? {
              full_name: prescriptionData.doctor_full_name,
            }
          : null,
