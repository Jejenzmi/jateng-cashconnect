...
export function ScheduleRosterTab() {
  ...
  const upsertSchedule = useMutation({
    mutationFn: async ({
      ...
      const existingRecord = await getApi("/generic-api");
  ...
  const copyPreviousWeek = useMutation({
    mutationFn: async () => {
      ...
      const prevSchedules = await getApi("/generic-api");
      ...
      for (const s of prevSchedules) {
        const newDate = format(addDays(parseISO(s.schedule_date), 7), "yyyy-MM-dd");
        
        // Check if record exists for the new date
        const existingRecord = await getApi("/generic-api");
