...
export default function AcademicActivities() {
  ...
  const fetchData = async () => {
    try {
      const [activitiesRes, deptsRes] = await Promise.all([
        fetch('/api/academic-activities').then(r => r.json()),
      ...
      toast({ title: "Error", description: "Gagal memuat data kegiatan akademik", variant: "destructive" });
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingActivity) {
        await fetch(`/api/academic-activities/${editingActivity.id}`, {
          method: 'PUT',
        ...
        await fetch('/api/academic-activities', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
