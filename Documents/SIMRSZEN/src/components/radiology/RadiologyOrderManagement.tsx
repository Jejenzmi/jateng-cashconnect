...
const RadiologyOrderManagement = () => {
  const [editingOrder, setEditingOrder] = useState<RadiologyOrder | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  
  const [formData, setFormData] = useState<Omit<RadiologyOrder, 'id' | 'patient' | 'doctor' | 'visit' | 'exam' | 'result' | 'createdAt' | 'updatedAt'>>({
    patientId: '',
    doctorId: '',
    visitId: '',
    examId: '',
  ...
  const createMutation = useMutation({
    mutationFn: async (orderData: Omit<RadiologyOrder, 'id' | 'patient' | 'doctor' | 'visit' | 'exam' | 'result' | 'createdAt' | 'updatedAt'>) => {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/radiology-orders`, orderData);
    ...
    onSuccess: () => {
      ...
      toast.success(editingOrder ? 'Pesanan radiologi berhasil diperbarui' : 'Pesanan radiologi berhasil ditambahkan');
    },
    onError: (error: any) => {
      console.error('Error saving radiology order:', error);
      const errorMessage = error.response?.data?.message || 
                          (error.response?.data?.errors?.map((e: any) => e.message).join(', ') || 
                          'Gagal menyimpan data pesanan radiologi');
      toast.error(errorMessage);
    }
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, ...orderData }: RadiologyOrder) => {
      const response = await axios.put(`${import.meta.env.VITE_API_URL}/radiology-orders/${id}`, orderData);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['radiologyOrders'] });
      ...
      toast.success('Pesanan radiologi berhasil diperbarui');
    },
    onError: (error: any) => {
      console.error('Error updating radiology order:', error);
      const errorMessage = error.response?.data?.message || 
                          (error.response?.data?.errors?.map((e: any) => e.message).join(', ') || 
                          'Gagal memperbarui data pesanan radiologi');
      toast.error(errorMessage);
    }
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await axios.delete(`${import.meta.env.VITE_API_URL}/radiology-orders/${id}`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['radiologyOrders'] });
      toast.success('Pesanan radiologi berhasil dihapus');
    },
    onError: (error: any) => {
      console.error('Error deleting radiology order:', error);
      toast.error(error.response?.data?.message || 'Gagal menghapus data pesanan radiologi');
    }
  });
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Yakin ingin menghapus data pesanan radiologi ini? Data akan dihapus secara permanen.')) {
      deleteMutation.mutate(id);
    }
  };

  if (isError) return <div className="p-4 text-red-500">Error saat memuat data pesanan radiologi</div>;

  return (
    <div className="container mx-auto py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Manajemen Pesanan Radiologi</h1>
        <button 
          onClick={() => {
            setEditingOrder(null);
            setIsModalOpen(true);
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {orders?.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{order.exam.name}</td>
          ...
          {(!orders || orders.length === 0) && (
            <div className="text-center py-8 text-gray-500">
              Belum ada data pesanan radiologi
            </div>
          )}
        </div>
      )}
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <h2 className="text-xl font-bold mb-4">
                {editingOrder ? 'Edit Pesanan Radiologi' : 'Tambah Pesanan Radiologi Baru'}
              </h2>
...
export default RadiologyOrderManagement;