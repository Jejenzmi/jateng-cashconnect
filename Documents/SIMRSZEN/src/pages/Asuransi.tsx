...
export default function Asuransi() {
  ...
  const { data: providers = [] } = useQuery({
    ...
    queryFn: async () => {
      const data = await getApi<InsuranceProvider[]>`
  ...
  const { data: claims = [], isLoading } = useQuery({
    queryKey: ["insurance-claims", searchTerm, statusFilter, typeFilter],
    queryFn: async () => {
      let querySql = `
        SELECT 
          ic.id,
          ...
          ) FROM patients WHERE id = ic.patient_id) AS patients,
          (SELECT json_build_object(
            ...
            'insurance_providers', (
              SELECT json_build_object(
                ...
                'name', ip.name,
                'type', ip.type
              ) FROM insurance_providers ip WHERE ip.id = pi.insurance_provider_id
            )
          ) FROM patient_insurances pi WHERE pi.id = ic.patient_insurance_id) AS patient_insurances
      ...
      if (searchTerm) {
        querySql += ` AND (ic.claim_number ILIKE $${paramIndex} OR p.full_name ILIKE $${paramIndex} OR p.medical_record_number ILIKE $${paramIndex})`;
        params.push(`%${searchTerm}%`);
        paramIndex++;
      }

      if (statusFilter !== "all") {
        querySql += ` AND ic.status = $${paramIndex}`;
        params.push(statusFilter);
        paramIndex++;
      }

      if (typeFilter !== "all") {
        querySql += ` AND ip.type = $${paramIndex}`;
        params.push(typeFilter);
        paramIndex++;
      }

      querySql += ` ORDER BY ic.priority_order DESC, ic.claim_date DESC`;

      const data = await getApi<InsuranceClaim[]>(querySql);
      return data;
    },
  });
  ...
  const updateClaimStatus = useMutation({
    mutationFn: async ({ id, status, notes }: { id: string; status: ClaimStatus; notes?: string }) => {
  ...
  const filteredClaims = claims.filter(claim => {
    const matchesSearch = 
      ...
      claim.patients?.medical_record_number.toLowerCase().includes(searchTerm.toLowerCase());

    const matchesStatus = statusFilter === "all" || claim.status === statusFilter;
    const matchesType = typeFilter === "all" || 
      (claim.patient_insurances?.insurance_providers?.type === typeFilter);
