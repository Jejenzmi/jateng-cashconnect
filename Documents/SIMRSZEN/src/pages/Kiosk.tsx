...
export default function Kiosk() {
  ...
  const { data: departments = [] } = useQuery({
    ...
    queryFn: async () => {
      const departments = await getApi<Department[]>`
        SELECT id, name, code
        FROM departments
  ...
  const { data: patients = [], isLoading: searchingPatients } = useQuery({
    ...
    queryFn: async () => {
      ...
      const patients = await getApi<Patient[]>`
        SELECT id, full_name, medical_record_number, nik, phone
        FROM patients
        WHERE 
          medical_record_number ILIKE ${`%${searchQuery}%`} OR
          nik ILIKE ${`%${searchQuery}%`} OR
  ...
  const { data: appointments = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const appointments = await getApi<Appointment[]>`
        SELECT 
          a.id,
          a.appointment_date,
          a.appointment_time,
          a.status,
          jsonb_build_object('full_name', d.full_name) AS doctors,
          jsonb_build_object('name', dept.name) AS departments
        FROM appointments a
        LEFT JOIN doctors d ON a.doctor_id = d.id
        LEFT JOIN departments dept ON a.department_id = dept.id
        WHERE a.patient_id = ${selectedPatient.id}
  ...
  const generateTicket = useMutation({
    mutationFn: async () => {
      ...
      // Get last ticket number for the service
      const lastTicket = await getApi<{ ticket_number: string }[]>`
        SELECT ticket_number
        FROM queue_tickets
        WHERE queue_date = ${today} AND service_type = ${selectedService.id}
      ...
      if (lastTicket.length > 0) {
        const lastNum = parseInt(lastTicket[0].ticket_number.replace(/\D/g, "")) || 0;
  ...
  const checkInAppointment = useMutation({
    mutationFn: async (appointmentId: string) => {
      ...
      // Get last ticket number for rawat_jalan
      const lastTicket = await getApi<{ ticket_number: string }[]>`
        SELECT ticket_number
        FROM queue_tickets
        WHERE queue_date = ${today} AND service_type = 'rawat_jalan'
        ORDER BY created_at DESC
        LIMIT 1
      `;
      ...
      <div className="bg-white/10 backdrop-blur-md border-b border-white/10 p-5 flex items-center justify-between">
        <div className="flex items-center gap-5">
          <div className="bg-white rounded-xl p-2 shadow-lg">
            <img src={rsudLogo} alt="SIMRS ZEN" className="h-14 w-auto" />
          </div>
