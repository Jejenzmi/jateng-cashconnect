...
export function useTelemedicineData() {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const todaySessions = (data || []).filter((s: TelemedicineSession) => 
        s.scheduled_start.startsWith(today)
      );
      
      const completedSessions = todaySessions.filter((s: TelemedicineSession) => s.status === "completed");
      const totalDuration = completedSessions.reduce((sum: number, s: TelemedicineSession) => sum + (s.duration_minutes || 0), 0);

      setStats({
        today: todaySessions.length,
        waiting: todaySessions.filter((s: TelemedicineSession) => s.status === "waiting" || s.status === "in_progress").length,
  ...
  const createSession = async (appointmentId: string) => {
    try {
      ...
      const appointmentResult = await getApi("/generic-api");
  ...
  const endSession = async (sessionId: string, notes?: string) => {
    try {
      ...
      const sessionResult = await getApi("/generic-api");
