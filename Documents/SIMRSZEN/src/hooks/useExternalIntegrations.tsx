...
export function useExternalIntegrations() {
  ...
  const updateIntegration = useMutation({
    mutationFn: async ({ key, value }: { key: string; value: any }) => {
      ...
      const existing = await getApi("/generic-api");
  ...
  const testConnection = useMutation({
    mutationFn: async ({ integration, config }: { integration: string; config: any }) => {
      ...
      // In the PostgreSQL implementation, we'll call the backend API
      // which handles the actual communication with external services
      ...
      if (!response.ok) {
        throw new Error(`Failed to connect to ${integration}: ${response.statusText}`);
      }
