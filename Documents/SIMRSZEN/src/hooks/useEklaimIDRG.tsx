...
async function callEklaimIDRG(action: string, data: Record<string, any> = {}) {
  ...
  // which handles the actual communication with BPJS
  const response = await fetch('/api/integrations/bpjs/eklaim', {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`E-Klaim API error: ${response.statusText}`);
  }
