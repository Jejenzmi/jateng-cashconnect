...
async function callPACS(action: string, data: Record<string, any> = {}) {
  ...
  // which handles the actual communication with PACS systems
  const response = await fetch('/api/integrations/pacs', {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`PACS API error: ${response.statusText}`);
  }
